...
type InterviewType = "written" | "face_to_face" | "practical" | "screening";

type Props = {
  id?: number;
};

export function InterviewForm({ id }: Props) {
  ...
  const [interviewType, setInterviewType] = useState<InterviewType>("written");
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/interviews/${id}`)
      ...
      .then((d) => {
        ...
        setSlug(it.slug ?? "");
        setInterviewType(it.interviewType ?? "written");
  ...
  async function remove() {
    ...
    if (res.ok) router.push("/admin/interviews");
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">
          {id ? "Edit interview" : "New interview"}
        </h1>
        <p className="text-sm text-muted mt-1">
          {id ? "Update the interview details below" : "Fill in the details for the new interview"}
        </p>
      </div>
